...
export default function Confirmation({
  ...
  return (
    <div className="flex w-full max-w-[560px] flex-col items-center gap-6 py-10">
      ...
      <p className="w-full rounded-xl border border-dashed border-border-input bg-surface-muted px-4 py-3 text-center text-xs text-slate">
      ...
      <button
        type="button"
        onClick={onDone}
        className="flex h-14 w-full flex-col items-center justify-center gap-0.5 rounded-full text-white shadow-[0px_4px_10px_0px_rgba(0,0,0,0.05),0px_10px_120px_0px_rgba(15,73,106,0.1)]"
      ...
      <div className="flex w-full flex-col gap-2 rounded-xl border border-dashed border-border-input px-5 py-4">
        ...
        <ul className="flex flex-col gap-1.5 text-sm text-slate">
          ...
          {invoice.timeRange && <li>Cancel free up to 4 hours before {invoice.timeRange.split('–')[0].trim()}.</li>}
        </ul>
      </div>
    </div>
  )
