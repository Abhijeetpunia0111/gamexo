"""Prove the SMTP credentials work, before a real booking depends on them.
...
async def run(to: str, slug: str | None, template: str) -> int:
    if not is_configured():
        print("SMTP is not configured — set SMTP_HOST (and friends) in apps/api/.env.")
        return 2
    ...
    if settings.smtp_redirect_all_to:
        print(f"redirect : all mail -> {settings.smtp_redirect_all_to}")
    print(f"to       : {to}")
    ...
    async with tenant_session(_uuid.UUID(tenant_id)) as session:
        ...
        if template == "payment":
            ...
            subject = f"{tenant_settings.business_name} — SMTP test"
            text = (
                "This is a test message from gamexo.\n\n"
                f"Sent at {when:%d %b %Y %H:%M} UTC.\n"
                "If you are reading this, the SMTP credentials work."
            )
...
def _hint(exc: Exception) -> None:
    ...
    if "username and password not accepted" in text or "5.7.8" in text:
        ...
        print(
            "\nTimed out. Port 587 wants SMTP_STARTTLS=true and SMTP_USE_TLS=false;\n"
            "port 465 wants the opposite. Outbound SMTP is also blocked on many\n"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Send one test email through the configured SMTP server")
